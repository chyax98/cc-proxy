...
import type { RequestBody, RequestHeaders, SystemBlock, Metadata } from '../types.js';
...
export class ClaudeCodePlugin extends ProtocolPlugin {
  ...
  readonly description = 'Transform requests to Claude Code CLI format';

  // Claude Code system 模板
  private static readonly SYSTEM_TEMPLATE: SystemBlock[] = [
    {
      type: 'text',
  ...
  // Session 缓存
  private cachedSession: Metadata | null = null;
  private sessionExpireTime = 0;
  private readonly SESSION_TTL = 12 * 60 * 60 * 1000; // 12小时

  /**
   * 检测是否为需要转换的请求
   */
  detect(body: RequestBody, headers: RequestHeaders): boolean {
   ...
   * 转换请求体
   */
  transform(body: RequestBody, headers: RequestHeaders): RequestBody {
